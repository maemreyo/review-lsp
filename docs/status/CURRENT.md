...
Baseline release: `review-lsp@0.1.0-alpha.6`.
...
| Project ownership / routing | Admitted for the Alpha.6 `files` / `include` / `exclude` / relative-`extends` subset | Ownership is evidence-based, not proximity-based; ambiguous/unsupported routing fails closed; `allowJs` is admitted only as the minimal include-membership modifier; default file discovery/package-based `extends` remain deferred |
...
Alpha.6 ownership/routing authority is `docs/design-notes/ALPHA6_PROJECT_OWNERSHIP_ROUTING_MASTER_PLAN.md`. Alpha.5 project-reference authority remains preserved in `docs/design-notes/ALPHA5_PROJECT_REFERENCE_MASTER_PLAN.md`; Alpha.4 semantic-operation authority remains preserved in `docs/design-notes/ALPHA4_SEMANTIC_IMPACT_MASTER_PLAN.md`. Later expansion must preserve the same candidate/environment/toolchain evidence boundaries.
