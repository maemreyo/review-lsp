...
async function scenario(options: { idleTtlMs?: number } = {}) {
  ...
  return { candidate, profile, state, manager, resolvingProject };
}
...
describe("semantic runtime reuse", () => {
  it("serves concurrent questions from one runtime", async () => {
    const { candidate, profile: admitted, state, manager, resolvingProject } = await scenario();
    ...
    const leases = await Promise.all([0, 1, 2, 3].map(() => manager.acquire({
      candidate, profile: admitted, stateDirectory: state, resolvingProject,
    })));
  ...
  it("reuses a warm runtime for a later question instead of rebuilding it", async () => {
    ...
    const first = await manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject });
    const firstSession = first.session;
    first.release();

    const second = await manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject });
  ...
  it("does not close a runtime while a question is still holding it", async () => {
    const { candidate, profile: admitted, state, manager, resolvingProject } = await scenario({ idleTtlMs: 10 });

    const holder = await manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject });
    const transient = await manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject });
    transient.release();
  ...
  it("closes an unreferenced runtime once it has been idle", async () => {
    const { candidate, profile: admitted, state, manager, resolvingProject } = await scenario({ idleTtlMs: 50 });

    const lease = await manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject });
    lease.release();
  ...
  it("refuses to hand out runtimes after disposal", async () => {
    ...
    await expect(manager.acquire({ candidate, profile: admitted, stateDirectory: state, resolvingProject }))
